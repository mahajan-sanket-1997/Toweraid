...
class _RegistrationDetailsState extends State<RegistrationDetails> {
  ...
  Future<void> _load() async {
    try {
      final rows = await sb!.rpc('get_registration_options');
      options = (rows as List)
          .map((r) => RegistrationOption.fromMap(Map<String, dynamic>.from(r)))
          .toList();
      if (options.isNotEmpty) selected = options.first;
    } catch (_) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Could not load society and flat details. Please try again.')),
        );
      }
    } finally {
  ...
  Future<void> _complete() async {
    ...
    try {
      await sb!.rpc('complete_resident_registration', params: {
        'society_id_input': selected!.societyId,
        'building_id_input': selected!.buildingId,
        'floor_id_input': selected!.floorId,
        'flat_id_input': selected!.flatId,
        'resident_type_input': residentType,
      });
      if (mounted) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Registration could not be completed: $e')),
        );
      }
    } finally {
  ...
  Widget pick(String label, String Function(RegistrationOption) f) {
    return DropdownButtonFormField<RegistrationOption>(
      value: selected,
      isExpanded: true,
      ...
      items: options
          .map((o) => DropdownMenuItem(value: o, child: Text(f(o))))
          .toList(),
      onChanged: saving ? null : (v) => setState(() => selected = v),
    );
  }
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text('Choose your society, tower, floor and flat.'),
                      const SizedBox(height: 20),
                      pick('Society', (o) => o.societyName),
                      const SizedBox(height: 16),
                      pick('Tower / Building', (o) => o.buildingName),
                      const SizedBox(height: 16),
                      pick('Floor', (o) => 'Floor ${o.floorNumber}'),
                      const SizedBox(height: 16),
                      pick('Flat', (o) => 'Flat ${o.flatNumber}'),
                      const SizedBox(height: 16),
                      DropdownButtonFormField<String>(
                        ...
                        onChanged: saving ? null : (v) => setState(() => residentType = v!),
                      ),
                      const SizedBox(height: 22),
                      FilledButton(
                        onPressed: saving || selected == null ? null : _complete,
                        child: saving
                            ? const SizedBox(
                                width: 22,
                                height: 22,
                      ...
                      const SizedBox(height: 8),
                      const Text(
                        'Development mode: selected residents are verified immediately.',
